...
{	
	...
	/**
	 * Config scope of notice
	 */
	public function defaultScope(){
		return array(
			'condition'=>'type = '.Article::ARTICLE_NOTICE,
		);	
	}
	/**
	 * Config status of notice
	 */
	...
	/**
	 * Config special
	 * SPECIAL_REMARK notice is notice that can be viewed at homepage
	 * SPECIAL_NOTICE notice is notice that can be viewed at notice
	 * SPECIAL_MARQUEE notice is notice that can be viewed at...
	 */
	const SPECIAL_REMARK=0;
	
	/**
	 * Config other attribute
	 ** INTRO_LENGTH number characters of notice displayed in introduction
	 ** INTRO_HOMEPAGE_LENGTH number characters of notice displayed in homepage
	 ** OTHER_NOTICE number of notice display in the other notice widget
	 ** LIST_NOTICE number of notice displayed in the list notice page
	 ** LIST_SEARCH number of notice displayed in the list of search page
	 ** PRESENT_CATEGORY
	 ** PRESENT_CATEGORY_EN 
	 */
	...
	const OTHER_NOTICE=5;
	const LIST_NOTICE=10;
	...
	/**
	 * Get update url of notice
	 * @return notice's update url
	 */
	public function getUpdate_url()
 	{
 		$url=Yii::app()->createUrl("admin/notice/update",array('id'=>$this->id));
		return $url;
 	}
	/**
	 * Get url of this notice
	 * @return string $url, the absoluted path of this notice
	 */
	public function getUrl()
 	{
 		$cat_alias=$this->category->alias;
 		$alias=$this->alias;
 		$url=Yii::app()->createUrl("notice/view",array('cat_alias'=>$cat_alias,'notice_alias'=>$alias));
		return $url;
 	}
	/*
	 * Get thumb of video
	...
	public function getThumb_url($type,$class){
		...
		if($this->introimage>0){
			...
			if(isset($image)){
				$src=$image->getThumb('Notice',$type);
				if( $image->title != '')	$alt=$image->title;
			}
			else {
				$src=Image::getDefaultThumb('Notice', $type);
			}
			return '<img class="'.$class.'" src="'.$src.'" alt="'.$alt.'">';
		}
		else {
			
			return '<img class="'.$class.'" src="'.Image::getDefaultThumb('Notice', $type).'" alt="'.$alt.'">';
		}
	}
	/**
	 ...
	 */
 	...
 	/** 
 	 * Get name of category of this notice
	...
	public function getList_similar(){
		...
		$list = array_diff ( explode ( ',', $this->list_suggest ), array ('' ) );
			...
			foreach ($list as $id){
				...
				if(isset($notice)){
					$index++;
					if($index <= Setting::s('LIMIT_SIMILAR_NOTICE','Notice'))
						$result[]=Notice::model()->findByPk($id);
				}
				else{
			...
			$criteria->limit=Setting::s('LIMIT_SIMILAR_NOTICE','Notice');
			$result=Notice::model()->findAll($criteria);		
		}
		return $result;
	}	
 	/*
	 * Get all specials of class Notice
	 * Used in dropdownlist when create or update notice
	 */
	static function getList_label_specials()
 	{
	return array(
			self::SPECIAL_REMARK=>'Hiển thị trong phần thông báo nổi bật',
		);
 	}
 	/**
	...
	/**
	 * Get thumb image of this notice
	 * @param string $type, type of needed thumb image
	 */
	public function getImage($type){
		$image=Image::model()->findByPk($this->introimage);
		if(isset($image)){
			$url='<img class="img" src="'.$image->getThumb('Notice',$type).'" alt="'.$image->title.'" />';
		}
		else {
			$url='<img class="img" src="'.Image::getDefaultThumb('Notice',$type).'" />';
		}
			return $url;	
	}
	/**
	...
	/**
	 * Function validate notice title, don't allow add a notice has same title with an existing notice
	...
	public function beforeSave()
	{
		if(parent::beforeSave())
		{
			if($this->isNewRecord)
			{
				...
				$this->status=Notice::STATUS_ACTIVE;
				$alias=iPhoenixString::createAlias($this->title);
				while(sizeof(Notice::model()->findAll('alias = "'.$alias.'"'))>0){
					$suffix=rand(1,99);
					$alias =$alias.'-'.$suffix;
				}
				$this->alias=$alias;				
			...
			$this->type=Article::ARTICLE_NOTICE;
			//Encode fulltext
	...
	public function search() {
		...
		if (!Yii::app ()->user->checkAccess ( 'notice_update') && Yii::app()->controller->id == 'notice' && Yii::app()->controller->action->id == 'index') {
	...
	{
		$list_notice=$this->findAll(array(
	...
	/**
	 * Copy notice
	 * @param integer $id, the ID of notice to be copied
	 */
	static function copy($id) {
		...
		if ($command->execute ()) {
			...
			$model = Notice::model ()->findByPk ( $copy_id );
			$model->scenario = 'copy';
