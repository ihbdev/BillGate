 ...
 */
/**
 * This is the model class for table "primarySchool".
 *
 */
class PrimarySchool extends CActiveRecord
{	
	/**
	...
	/**
	 * Config scope of primarySchool
	 */
	public function defaultScope(){
		return array(
			'condition'=>'type = '.Article::ARTICLE_PRIMARYSCHOOL,
		);	
	}
	/**
	 * Config status of primarySchool
	 */
	...
	/**
	 * Config special
	 * SPECIAL_REMARK primarySchool is primarySchool that can be viewed at homepage
	 * SPECIAL_NOTICE primarySchool is primarySchool that can be viewed at notice
	 * SPECIAL_MARQUEE primarySchool is primarySchool that can be viewed at...
	 */
	const SPECIAL_REMARK=0;
	
	/**
	 * Config other attribute
	 ** INTRO_LENGTH number characters of primarySchool displayed in introduction
	 ** INTRO_HOMEPAGE_LENGTH number characters of primarySchool displayed in homepage
	 ** OTHER_PRIMARYSCHOOL number of primarySchool display in the other primarySchool widget
	 ** LIST_PRIMARYSCHOOL number of primarySchool displayed in the list primarySchool page
	 ** LIST_SEARCH number of primarySchool displayed in the list of search page
	 ** PRESENT_CATEGORY
	 ** PRESENT_CATEGORY_EN 
	 */
	...
	const OTHER_PRIMARYSCHOOL=5;
	const LIST_PRIMARYSCHOOL=10;
	...
	/**
	 * Get update url of primarySchool
	 * @return primarySchool's update url
	 */
	public function getUpdate_url()
 	{
 		$url=Yii::app()->createUrl("admin/primarySchool/update",array('id'=>$this->id));
		return $url;
 	}
	/**
	 * Get url of this primarySchool
	 * @return string $url, the absoluted path of this primarySchool
	 */
	public function getUrl()
 	{
 		$cat_alias=$this->category->alias;
 		$alias=$this->alias;
 		$url=Yii::app()->createUrl("primarySchool/view",array('cat_alias'=>$cat_alias,'primarySchool_alias'=>$alias));
		return $url;
 	}
	/*
	 * Get thumb of video
	...
	public function getThumb_url($type,$class){
		...
		if($this->introimage>0){
			...
			if(isset($image)){
				$src=$image->getThumb('PrimarySchool',$type);
				if( $image->title != '')	$alt=$image->title;
			}
			else {
				$src=Image::getDefaultThumb('PrimarySchool', $type);
			}
			return '<img class="'.$class.'" src="'.$src.'" alt="'.$alt.'">';
		}
		else {
			
			return '<img class="'.$class.'" src="'.Image::getDefaultThumb('PrimarySchool', $type).'" alt="'.$alt.'">';
		}
	}
	/**
	 ...
	 */
 	...
 	/** 
 	 * Get name of category of this primarySchool
	...
	public function getList_similar(){
 	...
 	/*
	 * Get all specials of class PrimarySchool
	 * Used in dropdownlist when create or update primarySchool
	 */
	...
	/**
	 * Get thumb image of this primarySchool
	 * @param string $type, type of needed thumb image
	 */
	public function getImage($type){
		$image=Image::model()->findByPk($this->introimage);
		if(isset($image)){
			$url='<img class="img" src="'.$image->getThumb('PrimarySchool',$type).'" alt="'.$image->title.'" />';
		}
		else {
			$url='<img class="img" src="'.Image::getDefaultThumb('PrimarySchool',$type).'" />';
		}
			return $url;	
	}
	/**
	 ...
	 * @return PrimarySchool the static model class
	 */
	...
	/**
	 * Function validate primarySchool title, don't allow add a primarySchool has same title with an existing primarySchool
	...
	public function validateUniqueAlias() {
		...
		if ($this->id > 0)
			$criteria->addCondition ( 'id <> ' . $this->id );
		$list = PrimarySchool::model ()->findAll ( $criteria );
		if (sizeof ( $list ) > 0)
			return false;
		else
			return true;
	...
	public function beforeSave()
	{
		if(parent::beforeSave())
		{
			if($this->isNewRecord)
			{
				...
				$this->status=PrimarySchool::STATUS_ACTIVE;
				$alias=iPhoenixString::createAlias($this->title);
				while(sizeof(PrimarySchool::model()->findAll('alias = "'.$alias.'"'))>0){
					$suffix=rand(1,99);
					$alias =$alias.'-'.$suffix;
				}
				$this->alias=$alias;				
			...
			$this->type=Article::ARTICLE_PRIMARYSCHOOL;
			//Encode fulltext
	...
	public function search() {
		...
		if (!Yii::app ()->user->checkAccess ( 'primarySchool_update') && Yii::app()->controller->id == 'primarySchool' && Yii::app()->controller->action->id == 'index') {
	...
	{
		$list_primarySchool=$this->findAll(array(
		...
		$titles=array();
		foreach($list_primarySchool as $primarySchool)
			$titles[]=$primarySchool->title;
			return $titles;
	}
	/**
	...
	/**
	 * Copy primarySchool
	 * @param integer $id, the ID of primarySchool to be copied
	 */
	static function copy($id) {
		...
		if ($command->execute ()) {
			...
			$model = PrimarySchool::model ()->findByPk ( $copy_id );
			$model->scenario = 'copy';
