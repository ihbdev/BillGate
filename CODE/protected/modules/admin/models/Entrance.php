...
{	
	...
	/**
	 * Config scope of entrance
	 */
	public function defaultScope(){
		return array(
			'condition'=>'type = '.Article::ARTICLE_ENTRANCE,
		);	
	}
	/**
	 * Config status of entrance
	 */
	...
	/**
	 * Config special
	 * SPECIAL_REMARK entrance is entrance that can be viewed at homepage
	 * SPECIAL_NOTICE entrance is entrance that can be viewed at notice
	 * SPECIAL_MARQUEE entrance is entrance that can be viewed at...
	 */
	const SPECIAL_REMARK=0;
	
	/**
	 * Config other attribute
	 ** INTRO_LENGTH number characters of entrance displayed in introduction
	 ** INTRO_HOMEPAGE_LENGTH number characters of entrance displayed in homepage
	 ** OTHER_ENTRANCE number of entrance display in the other entrance widget
	 ** LIST_ENTRANCE number of entrance displayed in the list entrance page
	 ** LIST_SEARCH number of entrance displayed in the list of search page
	 ** PRESENT_CATEGORY
	 ** PRESENT_CATEGORY_EN 
	 */
	...
	const OTHER_ENTRANCE=5;
	const LIST_ENTRANCE=10;
	...
	/**
	 * Get update url of entrance
	 * @return entrance's update url
	 */
	public function getUpdate_url()
 	{
 		$url=Yii::app()->createUrl("admin/entrance/update",array('id'=>$this->id));
		return $url;
 	}
	/**
	 * Get url of this entrance
	 * @return string $url, the absoluted path of this entrance
	 */
	public function getUrl()
 	{
 		$cat_alias=$this->category->alias;
 		$alias=$this->alias;
 		$url=Yii::app()->createUrl("entrance/view",array('cat_alias'=>$cat_alias,'entrance_alias'=>$alias));
		return $url;
 	}
	/*
	 * Get thumb of video
	...
	public function getThumb_url($type,$class){
		...
		if($this->introimage>0){
			...
			if(isset($image)){
				$src=$image->getThumb('Entrance',$type);
				if( $image->title != '')	$alt=$image->title;
			}
			else {
				$src=Image::getDefaultThumb('Entrance', $type);
			}
			return '<img class="'.$class.'" src="'.$src.'" alt="'.$alt.'">';
		}
		else {
			
			return '<img class="'.$class.'" src="'.Image::getDefaultThumb('Entrance', $type).'" alt="'.$alt.'">';
		}
	}
	/**
	 ...
	 */
 	...
 	/** 
 	 * Get name of category of this entrance
	...
	public function getList_similar(){
		...
		$list = array_diff ( explode ( ',', $this->list_suggest ), array ('' ) );
			...
			$index=0;
			foreach ($list as $id){
				$entrance=Entrance::model()->findByPk($id);
				if(isset($entrance)){
					$index++;
					if($index <= Setting::s('LIMIT_SIMILAR_ENTRANCE','Entrance'))
						$result[]=Entrance::model()->findByPk($id);
				}
				else{
			...
			$criteria->limit=Setting::s('LIMIT_SIMILAR_ENTRANCE','Entrance');
			$result=Entrance::model()->findAll($criteria);		
		}
		return $result;
	}	
 	/*
	 * Get all specials of class Entrance
	 * Used in dropdownlist when create or update entrance
	 */
	...
	/**
	 * Get thumb image of this entrance
	 * @param string $type, type of needed thumb image
	 */
	public function getImage($type){
		$image=Image::model()->findByPk($this->introimage);
		if(isset($image)){
			$url='<img class="img" src="'.$image->getThumb('Entrance',$type).'" alt="'.$image->title.'" />';
		}
		else {
			$url='<img class="img" src="'.Image::getDefaultThumb('Entrance',$type).'" />';
		}
			return $url;	
	}
	/**
	 ...
	 * @return Entrance the static model class
	 */
	...
	/**
	 * Function validate entrance title, don't allow add a entrance has same title with an existing entrance
	...
	public function validateUniqueAlias() {
		...
		if ($this->id > 0)
			$criteria->addCondition ( 'id <> ' . $this->id );
		$list = Entrance::model ()->findAll ( $criteria );
		if (sizeof ( $list ) > 0)
			return false;
		else
			return true;
	...
	public function beforeSave()
	{
		if(parent::beforeSave())
		{
			if($this->isNewRecord)
			{
				...
				$this->status=Entrance::STATUS_ACTIVE;
				$alias=iPhoenixString::createAlias($this->title);
				while(sizeof(Entrance::model()->findAll('alias = "'.$alias.'"'))>0){
					$suffix=rand(1,99);
					$alias =$alias.'-'.$suffix;
				}
				$this->alias=$alias;				
			...
			$this->type=Article::ARTICLE_ENTRANCE;
			//Encode fulltext
	...
	public function search() {
		...
		if (!Yii::app ()->user->checkAccess ( 'entrance_update') && Yii::app()->controller->id == 'entrance' && Yii::app()->controller->action->id == 'index') {
	...
	{
		$list_entrance=$this->findAll(array(
		...
		$titles=array();
		foreach($list_entrance as $entrance)
			$titles[]=$entrance->title;
			return $titles;
	}
	/**
	...
	/**
	 * Copy entrance
	 * @param integer $id, the ID of entrance to be copied
	 */
	static function copy($id) {
		...
		if ($command->execute ()) {
			...
			$model = Entrance::model ()->findByPk ( $copy_id );
			$model->scenario = 'copy';
