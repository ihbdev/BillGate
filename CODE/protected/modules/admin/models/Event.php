...
{	
	...
	/**
	 * Config scope of event
	 */
	public function defaultScope(){
		return array(
			'condition'=>'type = '.Article::ARTICLE_EVENT,
		);	
	}
	/**
	 * Config status of event
	 */
	...
	/**
	 * Config special
	 * SPECIAL_REMARK event is event that can be viewed at homepage
	 * SPECIAL_NOTICE event is event that can be viewed at notice
	 * SPECIAL_MARQUEE event is event that can be viewed at...
	 */
	const SPECIAL_REMARK=0;
	
	/**
	 * Config other attribute
	 ** INTRO_LENGTH number characters of event displayed in introduction
	 ** INTRO_HOMEPAGE_LENGTH number characters of event displayed in homepage
	 ** OTHER_EVENT number of event display in the other event widget
	 ** LIST_EVENT number of event displayed in the list event page
	 ** LIST_SEARCH number of event displayed in the list of search page
	 ** PRESENT_CATEGORY
	 ** PRESENT_CATEGORY_EN 
	 */
	...
	/**
	 * Get update url of event
	 * @return event's update url
	 */
	public function getUpdate_url()
 	{
 		$url=Yii::app()->createUrl("admin/event/update",array('id'=>$this->id));
		return $url;
 	}
	/**
	 * Get url of this event
	 * @return string $url, the absoluted path of this event
	 */
	public function getUrl()
 	{
 		$cat_alias=$this->category->alias;
 		$alias=$this->alias;
 		$url=Yii::app()->createUrl("event/view",array('cat_alias'=>$cat_alias,'event_alias'=>$alias));
		return $url;
 	}
	/*
	 * Get thumb of video
	...
	public function getThumb_url($type,$class){
		...
		if($this->introimage>0){
			...
			if(isset($image)){
				$src=$image->getThumb('Event',$type);
				if( $image->title != '')	$alt=$image->title;
			}
			else {
				$src=Image::getDefaultThumb('Event', $type);
			}
			return '<img class="'.$class.'" src="'.$src.'" alt="'.$alt.'">';
		}
		else {
			
			return '<img class="'.$class.'" src="'.Image::getDefaultThumb('Event', $type).'" alt="'.$alt.'">';
		}
	}
	/**
	 ...
	 */
 	...
 	/** 
 	 * Get name of category of this event
	...
	public function getList_similar(){
		...
		$list = array_diff ( explode ( ',', $this->list_suggest ), array ('' ) );
			...
			foreach ($list as $id){
				...
				if(isset($event)){
					$index++;
					if($index <= Setting::s('LIMIT_SIMILAR_EVENT','Event'))
						$result[]=Event::model()->findByPk($id);
				}
				else{
			...
			$criteria->limit=Setting::s('LIMIT_SIMILAR_EVENT','Event');
			$result=Event::model()->findAll($criteria);		
		}
		return $result;
	}	
 	/*
	 * Get all specials of class Event
	 * Used in dropdownlist when create or update event
	 */
	static function getList_label_specials()
 	{
	return array(
			self::SPECIAL_REMARK=>'Hiển thị trong phần sự kiện nổi bật',
		);
 	}
 	/**
	...
	/**
	 * Get thumb image of this event
	 * @param string $type, type of needed thumb image
	 */
	public function getImage($type){
		$image=Image::model()->findByPk($this->introimage);
		if(isset($image)){
			$url='<img class="img" src="'.$image->getThumb('Event',$type).'" alt="'.$image->title.'" />';
		}
		else {
			$url='<img class="img" src="'.Image::getDefaultThumb('Event',$type).'" />';
		}
			return $url;	
	}
	/**
	...
	/**
	 * Function validate event title, don't allow add a event has same title with an existing event
	...
	public function beforeSave()
	{
		if(parent::beforeSave())
		{
			if($this->isNewRecord)
			{
				...
				while(sizeof(Event::model()->findAll('alias = "'.$alias.'"'))>0){
					$suffix=rand(1,99);
					$alias =$alias.'-'.$suffix;
				}
				$this->alias=$alias;				
			...
			$this->type=Article::ARTICLE_EVENT;
			//Encode fulltext
	...
	public function search() {
		...
		if (!Yii::app ()->user->checkAccess ( 'event_update') && Yii::app()->controller->id == 'event' && Yii::app()->controller->action->id == 'index') {
	...
	/**
	 * Copy event
	 * @param integer $id, the ID of event to be copied
	 */
	static function copy($id) {
		...
		if ($command->execute ()) {
			...
			$model = Event::model ()->findByPk ( $copy_id );
			$model->scenario = 'copy';
