<!--begin inside content-->
	<div class="folder top">
		...
		<div class="folder-header">
			<h1>Quản trị tin tuyển sinh</h1>
			<div class="header-menu">
				<ul>
					<li><a class="header-menu-active new-icon" href=""><span>Chỉnh sửa tin tuyển sinh <?php echo $model->title;?></span></a></li>					
				</ul>
			</div>
		</div>
		<!--end title-->
		...
		<div>
            	<input type="button" class="button" value="Thêm tin tuyển sinh" style="width:180px;" onClick="parent.location='<?php echo Yii::app()->createUrl('admin/entrance/create')?>'"/>
                <input type="button" class="button" value="Danh sách tin tuyển sinh" style="width:180px;" onClick="parent.location='<?php echo Yii::app()->createUrl('admin/entrance')?>'"/>
			...
			<div class="fl">
				<ul>
					...
					<li>
                        	...
                        	<?php echo $form->dropDownList($model,'list_special',Entrance::getList_label_specials(),array('style'=>'width:250px','multiple' => 'multiple')); ?>
...
<ul>
	<?php 
		Yii::app()->clientScript->registerScript('search-entrance-suggest', "
			$('#entrance-search').submit(function(){
			$.fn.yiiGridView.update('entrance-list', {
				data: $(this).serialize()});
				return false;
			});");
	?>
	 <?php $form=$this->beginWidget('CActiveForm', array('method'=>'get','id'=>'entrance-search')); ?>
	 <li>
     	<?php echo $form->labelEx($model,'title'); ?>
       	<?php $this->widget('CAutoComplete', array(
        	            	'model'=>$suggest,
                         	'attribute'=>'title',
							'url'=>array('entrance/suggestTitle'),
							'htmlOptions'=>array(
								'style'=>'width:230px;',
								),
						)); ?>								
	...
	<li>
		...
		<?php echo $form->dropDownList($suggest,'catid',$list,array('style'=>'width:200px','name'=>'SuggestEntrance[catid]')); ?>
	</li>            
	<li>
	<label>&nbsp;</label> 
	<input type="submit" class="button" value="Lọc tin tuyển sinh">
	</li>
	<?php $this->endWidget(); ?>	
	<li>
	  <?php 
			$this->widget('iPhoenixGridView', array(
  				...
  				'summaryText'=>'Có {count} tin tuyển sinh',
  		...
  		function(){  	
			...
			jQuery.ajax({
				...
				success:function(data){
					$('#Entrance_list_suggest').val(data);
					hidenPopUp();
				},
				type:'POST',
				url:'<?php echo $this->createUrl('entrance/updateSuggest');?>',
				'cache':'false'});
			return false;
		});
</script>